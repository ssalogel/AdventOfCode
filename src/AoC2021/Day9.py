...
class Day9(Day):
    ...
    def find_low_points(
        ...
        for ix, point in enumerate(grid[0][1:-1]):
            if (
                point < grid[0][ix]
                and point < grid[0][ix + 2]
                and point < grid[1][ix + 1]
            ):
        ...
        for ix, point in enumerate(grid[-1][1:-1]):
            if (
                point < grid[-1][ix]
                and point < grid[-1][ix + 2]
                and point < grid[-2][ix + 1]
            ):
                low_points.append(point)
    ...
    def get_bassin(
        self, low_point: tuple[int, int], grid: list[list[int]]
    ) -> set[tuple[int, int]]:
        width = len(grid[0])
        height = len(grid)
    ...
    def part2(self, parsed_content) -> int:
        ...
        bassins = [
            self.get_bassin(low_point, parsed_content) for low_point in low_coords
        ]
