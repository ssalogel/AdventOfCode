...
class PassGraph(Graph):
    def get_all_routes_small_once(self, start: str, end: str):
        ...
        for connection in self.connections[start]:
            routes += self._recur_get_all_paths_small_once(
                connection, end, visited, [start, connection]
            )
        return routes

    def _recur_get_all_paths_small_once(
        self, pos: str, target: str, visited: set[str], path: list[str]
    ):
        if pos == target:
            return [path]
        possible_paths = []
        ...
        for connection in self.connections[pos]:
            if connection in visited:
                continue
            possible_paths += self._recur_get_all_paths_small_once(
                connection, target, visited, path + [connection]
            )
    ...
    def get_all_routes_one_small_twice(self, start: str, end: str):
        ...
        routes = []
        for connection in self.connections[start]:
            routes += self._recur_get_all_paths_one_small_twice(
                start, connection, end, visited, [start, connection], False
            )
        return routes

    def _recur_get_all_paths_one_small_twice(
        self,
        start: str,
        pos: str,
        target: str,
        ...
        for connection in self.connections[pos]:
            if connection in visited and extra_done:
                continue
            possible_paths += self._recur_get_all_paths_one_small_twice(
                start,
                connection,
                target,
                visited,
                path + [connection],
                connection in visited or extra_done,
            )
        return possible_paths
