...
class Day8(Day):
    ...
    def parse_content(self) -> list[list[str]]:
        return [instr.split(' ') for instr in self.content.split('\n')]
            ...
            if instr[0] == 'rect':
                ...
                if instr[2].startswith('x'):
                    for _ in range(int(instr[-1])):
                        # flake8: noqa
                        screen[0][offset], screen[1][offset], screen[2][offset], \
                        screen[3][offset], screen[4][offset], screen[5][offset] = screen[5][offset], screen[0][offset], screen[1][offset], \
                                                                                  screen[2][offset], screen[3][offset], screen[4][offset]
                else:
                    screen[offset].rotate(int(instr[-1]))
        return [list(d) for d in screen]

    def part1(self) -> int:
        return sum(map(sum, self.run_screen(50, 6, self.data_p1)))

    def part2(self) -> str:
        screen = self.run_screen(50, 6, self.data_p2)
