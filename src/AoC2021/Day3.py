...
class Day3(Day):
    ...
    def part2(self, parsed_content: list[str]) -> int:
        ...
        for pos in reversed(range(length)):
            ...
            if len(oxy_counts) != 1:
                oxy_most_common = (
                    oxy_counts[0][0] if oxy_counts[0][1] != oxy_counts[1][1] else 1
                )
                oxygen = list(
                    filter(lambda x: x & (1 << pos) == oxy_most_common << pos, oxygen)
                )
            ...
            if len(co2_counts) != 1:
                co2_least_common = (
                    co2_counts[1][0] if co2_counts[0][1] != co2_counts[1][1] else 0
                )
                co2 = list(
                    filter(lambda x: x & (1 << pos) == co2_least_common << pos, co2)
                )
